...
**サブフェーズまで特定した**（`measure-m`）: **MLPが61%**、attentionが21%、decision headが16%、
その他すべて（softmax/norm/embedding/scorer/decode）は**約1%**。instr/MACは1.79〜1.99で下限に近く、
wasmには`f32x4.mul`が420個あり**自動ベクトル化が既に効いている**。つまり**実装の作り直しでは速くならない。**

**壁時計が本質的な制約**（公式: update 40B、query 5B、目標スループット2B instructions/秒）:
...
**INT8/SIMDの最良（127B）でも約64秒。** 「ある程度の速度」にはならない。

**結論: MAC数を減らす以外に道が無い。蒸留が必須。**

**目標構成（encoder 6層 / hidden 768）を実測した**（53M params の合成pack）:

| 質問 | instructions |
|---|---|
| Noul / Choice / Score | 11.6B / 12.3B / 13.3B |
| **3質問合計** | **36.2B（update上限40Bの0.90倍、約18秒）** |

**F32のままでも上限内に収まる。** ここにINT8×3を当てると**約11.7B（約6秒）**で、
設計目標20Bを満たす見込み（この行は外挿。INT8カーネルの実装と再測定が次の作業）。

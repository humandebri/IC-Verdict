...
encoder層の内部まで分けて測った（`measure-m`、1質問あたり）:

| サブフェーズ | instructions | シェア | 理論MAC | instr/MAC |
|---|---|---|---|---|
| **mlp_up**（Wi射影） | 1,920,032,355 | **40.6%** | 1,073,741,824 | 1.79 |
| **attn**（QKV+窓内attention+Wo） | 1,002,968,392 | 21.2% | 約503M | 約1.99 |
| **mlp_down**（Wo射影） | 958,336,720 | 20.3% | 536,870,912 | 1.79 |
| mlp_act（GeLU） | 55,947,632 | 1.2% | 1,048,576 | 53.4 |
| attn_norm / resid | 数% | <1% | — | — |
| decision head | 742,253,057 | 15.7% | 805,306,368 | 0.92 |
| **その他すべて**（embedding・final_norm・scorer・decode） | 約52M | **約1.1%** | — | — |

**律速はMLPで、全体の61%を占める。** attentionが21%、decision headが16%。
softmax・norm・residual・gather・decodeは**合計1%程度**で、最適化しても意味がない。

instr/MACは1.79〜1.99で、**すでに下限に近い**。wasmを逆アセンブルすると
`f32x4.mul` が420個、`v128.load` が799個あり、**自動ベクトル化が既に効いている**。
したがって「実装が遅いから直せば速くなる」余地はほとんどない。
...
- **本当の律速はハードウェアではなく命令スループットである。** 実装効率はすでに下限近く（1.79〜1.99 instr/MAC、
  MLPが61%）。つまり**アルゴリズムを賢くしても速くならない**。速くする唯一の道は
  **MAC数そのものを減らす**こと、すなわちモデルを小さくすることである。

//! Canonical contiguous F32 pack for the openJev/GLiClass backend.
//!
//! Same rules as the Laya pack: tensors are stored back to back in the manifest's
//! order, each is length- and SHA-256-checked before it enters the model, and a
//! real (non-test) pack pins an immutable 40-hex source revision.
use crate::{expected_tensors,VerdictConfig,VerdictModel};
use candle_core::{Device,DType,Tensor};
use ic_laya_core::{hash,BackendKind,Digest,Error,Result};
use serde::{Deserialize,Serialize};
use std::collections::BTreeMap;

pub const FORMAT:&str="ic-verdict-f32-pack-v1";
